...
class AiMaxRoundsTest {
    ...
    fun aStoredBudgetIsUsedAsWritten() {
        val prefs = FakePrefs()
        prefs.putInt(PrefKeys.AI_MAX_ROUNDS, 24)
    ...
    fun aBudgetBelowTheFloorIsRaisedToIt() {
        val prefs = FakePrefs()
        for (stored in listOf(Int.MIN_VALUE, -1, 0, 1, PrefKeys.AI_MAX_ROUNDS_MIN - 1)) {
            prefs.putInt(PrefKeys.AI_MAX_ROUNDS, stored)
            assertEquals(
                "stored $stored should clamp up to the floor",
                PrefKeys.AI_MAX_ROUNDS_MIN,
                prefs.aiMaxRounds(),
            )
        }
    }
